...
- `npm test` — runs Jest (6 suites: `src/tests/*.test.ts` + `src/tests/models.test.ts`). Config is `jest.config.js` (plain CJS, `ts-jest/presets/default-esm`). Do **not** recreate `jest.config.ts` or you'll need `ts-node`.
