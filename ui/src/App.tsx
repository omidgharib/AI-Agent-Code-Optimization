...
function App() {
  ...
  const submit = async (event: FormEvent) => {
    ...
    try {
      const response = await fetch("/api/jobs", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ projectPath, url: auditUrl, fix, dryRun: agentMode !== "apply", agentMode: fix ? agentMode : undefined, severity, provider, model, baseUrl, apiKey: provider === "aifa" ? apiKey : undefined, analysisModel: model, issueIds: [...selectedIssueIds], maxAiRequests: 10, maxAgentSeconds: 300, maxChangedFiles: 5, maxAgentTokens: 100000, maxCritical: 0, maxHigh: 1000, sarif: true }) });
      const data = await response.json();
  ...
  const applyApproved = async () => {
    ...
    setError("");
        const response = await fetch(`/api/jobs/${active.id}/patches/apply`, { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ indexes: [...approvedPatches] }) });
    const data = await response.json();
  ...
  return <div className="app-shell">
    <aside className="sidebar">
      ...
      <nav><a className="active" href="#overview"><span>◈</span>{t.overview}</a><a href="#architecture"><span>⌬</span>{lang === "fa" ? "معماری" : "Architecture"}</a><a href="#test-health"><span>✓</span>{lang === "fa" ? "سلامت تست" : "Test Health"}</a><a href="#trust"><span>⌾</span>{lang === "fa" ? "مرکز اعتماد" : "Trust Center"}</a><a href="#issues"><span>◇</span>{t.issues}</a><a href="#activity"><span>›_</span>{t.activity}</a><a href="#history"><span>↺</span>{t.history}</a></nav>
    ...
    <main>
      ...
      <section className="panel patch-review" data-testid="patch-review"><div className="panel-head"><div><h2>{rc.review}</h2><span>{report?.agent ? `${report.agent.requests} ${rc.requests} · ${report.agent.estimatedTokens ?? 0} ${rc.tokens}` : rc.noPatches}</span></div><button className="apply-approved" disabled={!active || active.status !== "completed" || !report?.patches.some((patch, index) => patch.status === "ready" && approvedPatches.has(index))} onClick={applyApproved}>{rc.applyApproved} ({report?.patches.filter((patch, index) => patch.status === "ready" && approvedPatches.has(index)).length ?? 0})</button></div>{report?.patches.length ? <div className="patch-list">{report.patches.map((patch, index) => { const sides = diffSides(patch.unifiedDiff); const ready = patch.status === "ready"; const approved = ready && approvedPatches.has(index); return <article key={`${patch.description}-${index}`} className={`patch-card ${approved ? "approved" : ""}`}><header><div><strong>{patch.description}</strong><small>{patch.touches.join(", ")} · {patch.status}</small></div><div><button disabled={!ready} title={patch.preflight?.error} onClick={() => setApprovedPatches((current) => { const next = new Set(current); next.add(index); return next; })}>{ready ? rc.approve : (lang === "fa" ? "نیازمند اصلاح" : "Needs repair")}</button><button onClick={() => setApprovedPatches((current) => { const next = new Set(current); next.delete(index); return next; })}>{rc.reject}</button></div></header>{patch.preflight?.error && <p className="patch-validation-error">{patch.preflight.error}</p>}<div className="diff-grid"><pre className="before">{sides.before}</pre><pre className="after">{sides.after}</pre></div></article>; })}</div> : <p className="review-empty">{rc.noPatches}</p>}</section>
