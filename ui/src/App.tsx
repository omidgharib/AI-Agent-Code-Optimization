...
const agentCopy = {
  fa: { title: "عامل هوش مصنوعی", provider: "ارائه‌دهنده", model: "مدل", refresh: "دریافت مدل‌ها", connecting: "در حال اتصال…", online: "متصل", offline: "در دسترس نیست", local: "محلی", reasoning: "استدلال", search: "جستجوی وب", keyMissing: "کلید API روی سرور تنظیم نشده", hint: "برای اصلاح خودکار، ارائه‌دهنده و مدل عامل را انتخاب کنید." },
  en: { title: "AI agent", provider: "Provider", model: "Model", refresh: "Load models", connecting: "Connecting…", online: "Connected", offline: "Unavailable", local: "Local", reasoning: "Reasoning", search: "Web search", keyMissing: "API key is not configured on the server", hint: "Choose the agent provider and model used for automatic fixes." },
} as const;
...
function App() {
  ...
  const connect = (job: Job) => {
    ...
    events.addEventListener("status", async (event) => {
      ...
      if (["completed", "failed", "cancelled"].includes(next.status)) {
        events.close();
        if (next.reportPath) { const response = await fetch(`/api/jobs/${next.id}/report`); if (response.ok) { setReport(await response.json()); setApprovedPatches(new Set()); } }
      }
    });
  };
  ...
  const submit = async (event: FormEvent) => {
    ...
    try {
      const response = await fetch("/api/jobs", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ projectPath, url: auditUrl, fix, dryRun: agentMode !== "apply", agentMode: fix ? agentMode : undefined, severity, provider, model, baseUrl, analysisModel: model, issueIds: [...selectedIssueIds], maxAiRequests: 10, maxAgentSeconds: 300, maxChangedFiles: 5, maxAgentTokens: 100000, maxCritical: 0, maxHigh: 1000, sarif: true }) });
      const data = await response.json();
  ...
  const loadJob = async (job: Job) => {
    connect(job);
    if (job.reportPath) { const response = await fetch(`/api/jobs/${job.id}/report`); if (response.ok) setReport(await response.json()); }
  };

  const applyApproved = async () => {
    if (!active || approvedPatches.size === 0) return;
    setError("");
    let response = await fetch(`/api/jobs/${active.id}/patches/apply`, { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ indexes: [...approvedPatches] }) });
    let data = await response.json();
    if (response.status === 428 && window.confirm(lang === "fa" ? "این تغییر دامنه اثر بزرگی دارد. تأیید تکمیلی می‌دهید؟" : "This change has a large blast radius. Grant additional approval?")) { response = await fetch(`/api/jobs/${active.id}/patches/apply`, { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ indexes: [...approvedPatches], elevatedApproval: true }) }); data = await response.json(); }
  ...
  return <div className="app-shell">
    <aside className="sidebar">
      ...
      <nav><a className="active" href="#overview"><span>◈</span>{t.overview}</a><a href="#seo-lab"><span>◎</span>{lang === "fa" ? "آزمایشگاه SEO" : "SEO Lab"}</a><a href="#trust"><span>⌾</span>{lang === "fa" ? "مرکز اعتماد" : "Trust Center"}</a><a href="#issues"><span>◇</span>{t.issues}</a><a href="#activity"><span>›_</span>{t.activity}</a><a href="#history"><span>↺</span>{t.history}</a></nav>
      <div className="privacy"><span className="pulse"/><div><strong>{t.safe}</strong><small>127.0.0.1</small></div></div>
    </aside>

    <main>
      <header><div><h1>{t.newAudit}</h1><p>{t.subtitle}</p></div><button className="language" data-testid="language-toggle" aria-label={lang === "fa" ? "Switch to English" : "تغییر به فارسی"} onClick={() => setLang(lang === "fa" ? "en" : "fa")}>{lang === "fa" ? "EN" : "فا"}</button></header>
      ...
      <section className="launch-card">
        <form onSubmit={submit}>
          ...
          <section className={`agent-settings ${fix ? "enabled" : ""}`}><div className="agent-heading"><div><strong>{a.title}</strong><p>{a.hint}</p></div><span className={modelOnline ? "online" : "offline"} role="status" aria-live="polite">{loadingModels ? a.connecting : modelOnline ? a.online : a.offline}</span></div><div className="agent-grid"><label>{a.provider}<select data-testid="provider-select" value={provider} onChange={(event) => { const next = event.target.value; setProvider(next); setBaseUrl(providers.find((item) => item.id === next)?.baseUrl ?? ""); }}>{providers.map((item) => <option key={item.id} value={item.id}>{item.label}{item.local ? ` · ${a.local}` : ""}</option>)}</select></label><label>{a.model}<select data-testid="model-select" value={model} onChange={(event) => setModel(event.target.value)} disabled={loadingModels}>{models.map((item) => <option key={item.id} value={item.id}>{item.id}</option>)}</select></label><button type="button" onClick={() => loadModels(provider)} disabled={loadingModels}>↻ {a.refresh}</button></div><label className="endpoint-field">{rc.endpoint}<input value={baseUrl} onChange={(event) => setBaseUrl(event.target.value)} dir="ltr"/></label>{models.find((item) => item.id === model) && <div className="model-detail"><code>{models.find((item) => item.id === model)?.realModel ?? model}</code>{models.find((item) => item.id === model)?.reasoning && <span>{a.reasoning}</span>}{models.find((item) => item.id === model)?.webSearch && <span>{a.search}</span>}</div>}{providers.find((item) => item.id === provider)?.keyRequired && !providers.find((item) => item.id === provider)?.keyConfigured && <p className="key-warning">{a.keyMissing}</p>}</section>
